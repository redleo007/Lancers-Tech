...
export const useSprintStore = create<SprintState>((set) => ({
  ...
  updateSprint: (id, patch) =>
    set((st) => ({ sprints: st.sprints.map((s) => (s.id === id ? { ...s, ...patch } : s)) })),
  removeSprint: (id) => set((st) => ({ sprints: st.sprints.filter((s) => s.id !== id) })),
  addTaskToSprint: (sprintId, taskId) =>
    set((st) => ({
      sprints: st.sprints.map((s) => (s.id === sprintId && !s.taskIds.includes(taskId) ? { ...s, taskIds: [...s.taskIds, taskId] } : s)),
    })),
  removeTaskFromSprint: (sprintId, taskId) =>
    set((st) => ({
      sprints: st.sprints.map((s) => (s.id === sprintId ? { ...s, taskIds: s.taskIds.filter((id) => id !== taskId) } : s)),
    })),
}))
